...
<?
		  ...
		  if ($Submit)
	  {
		...
		if($recommendation=="" or $ss="" or $spv_name=="" or $interaction_date=="" or $observation_date=="" or $week_=="0" or $customer_name=="" or $interaction_type=="0" or $product_type=="" or $product_detail=="" or $product_detail_agent=="" or $repeat=="0" or $s_date_prev_call=="" or $s_prev_agent=="" or hh_2=="" or $mm_2=="" or $drop_call=="0" or $notes_recording=="")
			{
			?>
            <script type="text/javascript">
		...
		$q_addTemp="insert into table_qm_iic_detail(recommendation,id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,repeat,date_prev_call,prev_agent,hh_handling_time,mm_handling_time,ss_handling_time,drop_call,notes_recording)
		  values('$s_recommendation','$id_qm','$spv_name','$interaction_date_save',getdate(),'$week_','$customer_name','$interaction_type','$product_type','$product_detail','$product_detail_agent','$repeat','$date_prev_call','$prev_agent','$hh_2','$mm_2','$ss_2','$drop_call','$s_notes_recording')";
		  $simpan_data=mssql_query($q_addTemp);
		 // echo "$q_addTemp";
		  
		  if ($simpan_data)
					 { 
		$q_addTemp2="update table_qm_iic set status_save='complete',date_saved=getdate() where id_qm='$id_qm'";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_iic_detail.php"
		</script> 
          <?php
		}
