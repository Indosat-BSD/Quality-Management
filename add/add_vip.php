...
<body>
<form type="text" action="<? $PHP_SELF; ?>" method="post" enctype="multipart/form-data" name="welcome" id="welcome" onKeyPress="return noenter()">
  <p align="center">&nbsp;</p>
  ...
  <thead>
    ...
    <?
	$stot="select count(*)as jum_s from db_qm.dbo.table_qm where status_save='not_complete' and id_pribadi_observer='$SES_hendi'";
//echo "$stot";
	...
	if ($searchButton)
	  {
		...
		if($jum_tot1<>"0")
			...
			
			{
					...
					//echo"$q_user";
					//echo"ini jumlah total data : $jum_tot1";
					$no=1;
					while($d_user=mssql_fetch_array($r_user))
						{
						$pribadi1=$d_user["id_data_pribadi"];
                 ...
                 // echo"<td style='text-align:center;'><a href='?index=add_inbound&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$full_name&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi1&channel=Inbound'>Inbound</a>
						  // &nbsp&nbsp| &nbsp
						// <a href='?index=add_inbound&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$full_name&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi1&channel=VOCE'>VOCE</a>"; 
				 // }else{ echo "<td></td>";}
					echo"<td style='text-align:center;'><a href='?index=add_vip&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$full_name&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi1&channel=Vip'>VIP</a>
						  &nbsp&nbsp| &nbsp
						<a href='?index=add_vip&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$full_name&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi1&channel=VOCE'>VOCE</a>
						&nbsp&nbsp";
					...
					echo "<tr class='content'>
						  ...
						  <td style='text-align:center;'><a href='?index=vip&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$lb&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi1&nu=$nu&channel=Vip'>VIP</a>
						    &nbsp&nbsp| &nbsp
						      <a href='?index=add_vip&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$lb&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi1&nu=$nu&channel=VOCE'>VOCE</a>";
  ...
  <table width="313" border="0">
    <tr>
	 ...
	 if(isset($searchButton) and $jum_tot1=="0"){
	 	if($pribadi!=''){
		  $sql = mssql_query("select * from cco.hrms.dbo.tb_data_pribadi where id_data_pribadi ='$pribadi'");
		//}else{
		  //$sql = mssql_query("select * from cco.hrms.dbo.tb_data_pribadi where id_data_pribadi ='$pribadi'");
		//}
...
$result = mssql_query( $query );
  ...
  <thead>
    <tr>
      <td colspan="6" style="border-style:none">
		<?=$channel!=''?'<h2 style="text-align:center; margin-right:20px;">Form '.$channel.'</h2><br>':''?>
	  </td>
    </tr>
    <tr bgcolor="#CCCCCC">
  ...
  N
	...
	<tr>
	  ...
	  <td><div align="left"><input name="sc6" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="y" <? if ($sc6=="y") {echo "checked";} else {echo "";} ?>/>
  Y
    <input name="sc6" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="n" <? if ($sc6=="n") {echo "checked";} else {echo "";} ?>/>
  N
	  </div></td>
	...
	<tr>	
	  ...
	  <td><div align="left"><input name="sc7" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="y" <? if ($sc7=="y") {echo "checked";} else {echo "";} ?>/>
  Y
    <input name="sc7" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="n" <? if ($sc7=="n") {echo "checked";} else {echo "";} ?>/>
  N
	  </div></td>
	...
	<tr>
      ...
      <td><div align="center">3</div></td>
	  <td><div align="left"><input name="sc8" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="y" <? if ($sc8=="y") {echo "checked";} else {echo "";} ?>/>
  Y
    <input name="sc8" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="n" <? if ($sc8=="n") {echo "checked";} else {echo "";} ?>/>
  N
	  </div></td>
	...
	<tr>
	  ...
	  <td><div align="left"><input name="sc9" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="y" <? if ($sc9=="y") {echo "checked";} else {echo "";} ?>/>
  Y
    <input name="sc9" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="n" <? if ($sc9=="n") {echo "checked";} else {echo "";} ?>/>
  N
	  </div></td>
	...
	<tr>
	  ...
	  <td>Hold</td>
      <td><div align="center">5</div></td>
	  <td><div align="left"><input name="sc10" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="y" <? if ($sc10=="y") {echo "checked";} else {echo "";} ?>/>
  Y
    <input name="sc10" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="n" <? if ($sc10=="n") {echo "checked";} else {echo "";} ?>/>
  N
	  </div></td>
	...
	<tr>
	  ...
	  <td><div align="left"><input name="sc11" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="y" <? if ($sc11=="y") {echo "checked";} else {echo "";} ?>/>
  Y
    <input name="sc11" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="n" <? if ($sc11=="n") {echo "checked";} else {echo "";} ?>/>
  N
	  </div></td>
	...
	<tr>
      ...
      <td><div align="center">2</div></td>
	  <td><div align="left"><input name="sc12" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="y" <? if ($sc12=="y") {echo "checked";} else {echo "";} ?>/>
  Y
    <input name="sc12" type="radio"  <? if ($pol=="n") {?> disabled  <?} else {} ?> value="n" <? if ($sc12=="n") {echo "checked";} else {echo "";} ?>/>
  N
	  </div></td>
	...
	<tr>
	  ...
	  <?
		...
		if($sc4=="y"){ $tot4="43"; }else{ $tot4=0; }
		if($sc5=="y"){ $tot5="5"; }else{ $tot5=0; }
		if($sc6=="y"){ $tot6="3"; }else{ $tot6=0; }
		if($sc7=="y"){ $tot7="15"; }else{ $tot7=0; }
		if($sc8=="y"){ $tot8="3"; }else{ $tot8=0; }
		if($sc9=="y"){ $tot9="7"; }else{ $tot9=0; }
  ...
  <table width="935" border="0">
    <tr>
		  ...
		  if ($Submit)
	  {
		...
		if($pol=="y" and ($userTextBox=="" or $namaTextBox=="" or $kd_division=="0" or $kd_departemen=="0" or $kd_unit=="0" or $sc1=="" or $sc2==""  or $sc3=="" or $sc4=="" or $sc5=="" or $sc6=="" or $sc7=="" or $sc8=="" or $sc9=="" or $sc10=="" or $sc11=="" or $sc12==""))
			{
				$status=1;
			?>
            <script type="text/javascript">			
		...
		$q_addTemp="insert into table_qm(q1,q2,q3,q4,q5,q6,q7,q8,q9,q10,q11,q12,save_q1,save_q2,save_q3,save_q4,save_q5,save_q6,save_q7,save_q8,save_q9,save_q10,save_q11,save_q12,status_save,id_pribadi_user,id_pribadi_observer,date_saved,tot_score,id_unit,lb,notes1,notes2,notes3,notes4,notes5,notes6,notes7,notes8,notes9,notes10,notes11,notes12,flag_status,channel,privacypol)
					values('$sc1','$sc2','$sc3','$sc4','$sc5','$sc6','$sc7','$sc8','$sc9','$sc10','$sc11','$sc12','$tot1','$tot2','$tot3','$tot4','$tot5','$tot6','$tot7','$tot8','$tot9','$tot10','$tot11','$tot12','not_complete','$pribadi','$SES_hendi','$date_saved','$save_total','$kd_unit','$id_lb','$s_notes1','$s_notes2','$s_notes3','$s_notes4','$s_notes5','$s_notes6','$s_notes7','$s_notes8','$s_notes9','$s_notes10','$s_notes11','$s_notes12','2','$channel','$pol')";
		mssql_query($q_addTemp);
		//echo "$q_addTemp";
		  
  		echo"<script type='text/javascript'>
			  window.location='?index=add_vip&div=cco'
		     </script> ";
	  	}
	  }
  ...
  <thead>
    <tr bgcolor="#CCCCCC">
      ...
      <th rowspan="3" width="2%" rowspan="2" class="head0"><div align="center">Channel</div></th>
      ...
      <th colspan="15" class="head0"><div align="center">Interaksi, Konfirmasi & Solusi </div></th>
      <th colspan="3" class="head0"><div align="center">Additional program </div></th>
      <th colspan="3" class="head0"><div align="center">Teknis berbicara/ Effective Communication</div></th>
	  ...
	  <th rowspan="3" class="head0"><div align="center">PrivacyPolicy</div></th>
    </tr>
	...
	<tr bgcolor="#CCCCCC">
	  ...
	  <th colspan="3" class="head0"><div align="center">Information/Notes</div></th>
	  ...
	  <th colspan="3" class="head0"><div align="center">Effective Communication</div></th>
	...
	<?
			$q_user="select a.*,b.nama,b.nik_karyawan, case when a.lb !='' then a.lb else b.nama end as lb 
				from db_qm.dbo.table_qm a
				inner join cco.hrms.dbo.tb_data_pribadi b on a.id_pribadi_user=cast(b.id_data_pribadi as varchar) 
				where  a.id_pribadi_observer='$SES_hendi' and a.status_save='not_complete'";
				//echo "$q_user";
				$r_user=mssql_query($q_user);
				$no2=1;
				while($d_user=mssql_fetch_array($r_user))
		...
		echo "<tr class='content'>
                <td>$no2</td>";
					...
					?>
					  <a href="<? echo "add_vip_detail.php?observer_name=$observer_name&id_qm=$id_qm&full_name=$full_name&observer=$observer&total_score=$score_tot"; ?>" onClick="return popup(this, 'notes')">
					<?
		   ...
		   echo "<td>";
					?>					  
					  <a href="<? echo "del_group.php?kode_del=add_vip&id_qm=$id_qm"; ?>" onclick="return confirmDelete2();">
					<?
					  echo "delete</a></td><td>";
					?>
					  <a href="<? echo "edit_vip_2.php?kode_edit=edit_vip&report_by=$report_by&date_eva1=$date_eva1&date_eva2=$date_eva2&id_qm=$id_qm&full_name=$full_name&spv_name=$spv_name&observer_name=$observname&interaction_date=$interaction_date2&hh=$hh&mm=$mm&observation_date=$observ&week_=$week_&customer_name=$customer_name&interaction_type=$interaction_type&product_type=$product_type&product_detail=$product_detail&product_detail_agent=$product_detail_agent&repeat=$repeat&date_prev_call=$date_prev_call2&prev_agent=$prev_agent&hh_2=$hh_2&mm_2=$mm_2&ss_2=$ss_2&notes1=$notes1&notes2=$notes2&notes3=$notes3&notes4=$notes4&notes5=$notes5&notes6=$notes6&notes7=$notes7&notes8=$notes8&notes9=$notes9&notes10=$notes10&notes11=$notes11&notes12=$notes12&sc1=$q1&sc2=$q2&sc3=$q3&sc4=$q4&sc5=$q5&sc6=$q6&sc7=$q7&sc8=$q8&sc9=$q9&sc10=$q10&sc11=$q11&sc12=$q12&userid=$userid&observer=$SES__NIK&date_saved=$date_saved&total_score=$score_tot"; ?>" onClick="return popup(this, 'notes');">
					<?
					 echo "Edit</a></td>
