...
$result11 = mssql_query( $query11 );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='report/export_email.php?st_rep=$st_rep&cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&cc_matrix=$cc_matrix&cc_m2=$cc_m2&cc_ftm=$cc_ftm&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region&skala=$skala&depart=$departemen_skala&unitSkala=$unit_skala&tipe_report=$tipe_report'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
...
?>
<table border="1" align="center" id="dyntable" class="table table-bordered table-infinite" width="100%">
<thead>
    <tr bgcolor="#CCCCCC">
    <th width="1%" rowspan="3"><div align="center">No</div></th>
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
					...
					$observname=$qqq["observer_name"];
					  ...
					  <a href="<? echo "edit_email.php?id_qm=$id_qm&full_name=$full_name&observer_name=$observname&skala=$skala2"; ?>" onClick="return popup(this, 'notes');">	
					  <?
					  	 echo "Edit</a></td>";
						 }			  
...
?>
  ...
  <thead>
    ...
    <tr>
      ...
      <td><?  
				  //edit hendi
      $queryhendi="[sp_reportqm_email]'table_qm_email','$tgl1','$tgl2','$report_by','$c_observer','%$nik_agent%','$cc_mentari','$cc_im3','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala','$st_rep'";
	  		...
	  		if ($select_region !="") {
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_email_detail c
					   ...
					   and a.id_unit like '%$kd_unit%' and  convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2' and a.flag_status=4";
	   				}
	  			if ($report_by=="Departement")
					  ...
					  and  convert(varchar,a.date_saved,112) between '$tgl1' and'$tgl2' and a.flag_status=4"; 
					}
			}
			else {
	  			if ($report_by=="Unit")
...
between '$tgl1' and '$tgl2' and a.flag_status=4 and a.id_unit like '%$departemen_skala%' and a.id_unit like '%$unit_skala%' AND a.id_qm in( select id_qm from 
...
inner join db_qm.dbo.table_qm_email_detail b with(nolock) on a.id_qm=b.id_qm
 where a.status_save='complete' and convert(varchar,a.date_saved,112) 
  between '$tgl1' and '$tgl2' and a.flag_status=4 and a.id_unit like '%$departemen_skala%' and a.id_unit like '%$unit_skala%'
...
<h4 class="widgettitle">Chart EMAIL</h4>
<div class="widgetcontent">
<?php
$chartdatay = round($prosen_q1_y).','.round($prosen_q2_y).','.round($prosen_q3_y).','.round($prosen_q4_y).','.round($prosen_q5_y).','.round($prosen_q6_y).','.round($prosen_q7_y).','.round($prosen_q8_y).','.round($prosen_q9_y).','.round($prosen_q10_y);
$chartdatan = round($prosen_q1_n).','.round($prosen_q2_n).','.round($prosen_q3_n).','.round($prosen_q4_n).','.round($prosen_q5_n).','.round($prosen_q6_n).','.round($prosen_q7_n).','.round($prosen_q8_n).','.round($prosen_q9_n).','.round($prosen_q10_n);
...
<!--HEADER-->
	...
	if($tipe_report=='summary'){
		...
		$a_user="[new_report_email]'table_qm_email','$report_by','$tgl1','$tgl2','$select_region','$st_rep','summary' ";
		...
		$a_user="[new_report_email]'table_qm_email','$report_by','$tgl1','$tgl2','$select_region','$st_rep','detail'";
