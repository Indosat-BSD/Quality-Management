...
from db_qm.dbo.table_qm_sli a inner join
...
$result11 = mssql_query( $query11 );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='report/export_sli.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&cc_matrix=$cc_matrix&cc_m2=$cc_m2&cc_ftm=$cc_ftm&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region&skala=$skala_cmb&depart=$departemen_skala&unitSkala=$unit_skala&skalacmb=$skala_cmb'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
	...
	if ($date_eva1 != "" && $date_eva2 != "") {	
	 ...
	 $itung="[sp_count3]'table_qm_sli','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region','$skala_cmb','$departemen_skala','$unit_skala'";
...
<tr bgcolor="#CCCCCC">
	  ...
	  <? if ($SES_KODE_LEVEL<>"mitra" and $SES_KODE_LEVEL<>"sli"){?>
	...
	<?
					$q_user="[sp_search3]'table_qm_sli','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$c_observer%','%$nik_agent%','$select_region','$skala_cmb','$departemen_skala','$unit_skala'";
					//echo"$q_user";
...
$sc_nfe=$sc_q1+$sc_q2+$sc_q3+$sc_q7+$sc_q8+$sc_q9+$sc_q10+$sc_q11+$sc_q12+$sc_q13+$sc_q14+$sc_q15+$sc_q16+$sc_q17+$sc_q18+$sc_q19;*/
 					 	$total_score=$save_q1+$save_q2+$save_q4+$save_q5+$save_q6+$save_q7+$save_q8+$save_q9+$save_q10+$save_q11+$save_q20;
						if ($total_score >= 90)
						{
						$skala="A";
						}
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
					  ...
					  <a href="<? echo "del_group.php?report_by=$report_by&kode_del=detail_sli&id_qm=$id_qm&kd_division=$kd_division&kd_departemen=$kd_departemen&kd_unit=$kd_unit&date_eva1=$date_eva1&date_eva2=$date_eva2&c_observer=$observer"; ?>" onclick="return confirmDelete2();">
					  <?
					  	 echo "delete</a></td>";
...
?>
					...
					<td><a href="showKalibrasi/show_sli.php?id=<? echo "$id";?>&id_qm=<? echo "$id_qm";?>&agree=<? echo "$agree";?>" onClick="return popup(this, 'notes');">	
					<? if ($id_kalibrasi != ""){
  ...
  <thead>
    ...
    <tr>
      ...
      <td><?  
	 //edit hendi
      $queryhendi="[sp_reportqm3]'table_qm_sli','$tgl1','$tgl2','$report_by','%$c_observer%','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala_cmb','$departemen_skala','$unit_skala'";
    ...
    <tr>
	  ...
	  <? 
	  			if ($report_by=="Unit")
					  ...
					  $stot_fcr="[sp_fcr_tot]'table_qm_sli','$report_by','$tgl1','$tgl2','%$kd_unit%','$select_region','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','%$skala%','%$departemen_skala%','%$unit_skala%'";
					 ...
					 /* $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_sli_detail c inner join db_qm.dbo.table_qm_sli a
					  			...
					  			and  convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2'";*/
						$stot_rep="[sp_repeat_tot]'table_qm_sli','$report_by','$tgl1','$tgl2','%$kd_unit%','$select_region','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','%$skala%','%$departemen_skala%','%$unit_skala%'";
	   				}
...
";   */
    ...
    <tr>
      ...
      <td><?  
			$stot_hh="[sp_handling3]'table_qm_sli','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
					//echo $stot_hh;
    ...
    <tr>
      <td>Drop Call (Y) </td>
      <td><?  
		$stot_d="[sp_dropcall3]'table_qm_sli','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
    ...
    <tr>
      ...
      <td>
	  <?
	  $score_avg=$q1_avg+$q2_avg+$q4_avg+$q5_avg+$q6_avg+$q20_avg+$q7_avg+$q8_avg+$q9_avg+$q10_avg+$q11_avg;
	  //echo"$score_avg";
