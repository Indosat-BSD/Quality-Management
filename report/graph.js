...
var Graph = Graph || (function($) {
	...
	Private.defaults = function() {
		return {
			//default options
			x: ['SLA', 'Greeting', 'Verify', 'Correct Solution', 'Twit Handling ', 'Akurasi Data ', 'Note & Info','FCR','Accuracy','Closing','Additional Program'],
			y: 10,
			attachTo: 'body',
	...
	Private.design = function(obj) {
		...
		styling.style[this.parseS(obj.id, '.labels')] = {
			...
			"font-size": self.labelFontSize || "10px",
					...
					for (var t = 0, len = numPoints; t < len; ++t) { //this lets us loop array td instead of lr with j
						if (t !== npm1) { //skip over spaces array
							...
							if (which === 0) {
								...
								//tooltip text
								E.tooltips += '<text style="color:#000;background-color:#fff;" id="' + self.id + '-point-' + (ref) +
	...
	$.fn.graphify = $.fn.graphify || function(options) {
		...
		var buttons = (function() {
			...
			for (var i = 0, len = types.length; i < len; ++i) {
				btns += '<button style="visibility:hidden;" id="' + id + '-graphify-button-' + types[i] + '">' +
