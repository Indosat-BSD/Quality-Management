...
<form action="<? $PHP_SELF ?>" method="post" name="satu" enctype="multipart/form-data" onKeyPress="return noenter()">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
	...
	<tr class="content">
      <td>Channel </td>
      <td><select name="select_channel">
			<?=($select_channel=='' or $select_channel=='0')?'<option value="0"></option>':'';?>
			<option value="Inbound" <?=($select_channel=='Inbound')?'Selected':'';?>>Inbound</option>
			<option value="VOCE" <?=($select_channel=='VOCE')?'Selected':'';?>>VOCE</option>
		 </select> </td>
    </tr>
	<tr class="content">
      <td></td>
	...
	<tr class="content">
      ...
      <td><select name="select_channel">
			...
			<option value="Inbound" <?=($select_channel=='Inbound')?'Selected':'';?>>Inbound</option>
			<option value="VOCE" <?=($select_channel=='VOCE')?'Selected':'';?>>VOCE</option>
			<option value="FMC" <?=($select_channel=='FMC')?'Selected':'';?>>FMC</option>
		 </select> </td>
    </tr>
	<tr class="content">
      <td>Region </td>
...
Regular Contact Center 
	...
	<tr class="content">
      <td>Channel </td>
      <td><select name="select_channel" onchange="satu.submit()">
			<?=($select_channel=='' or $select_channel=='0')?'<option value="0"></option>':'';?>
			<option value="Inbound" <?=($select_channel=='Inbound')?'Selected':'';?>>Inbound</option>
			<option value="VOCE" <?=($select_channel=='VOCE')?'Selected':'';?>>VOCE</option>
			<option value="FMC" <?=($select_channel=='FMC')?'Selected':'';?>>FMC</option>
...
where a.channel='$select_channel'
order by z.nama";
//echo"$q_quizCombo";
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		...
		if($jum_tot<>"0" and $report_by<>"0" and ($date_eva1<>"" and $date_eva2<>"")){
			echo "<a class='btn btn-warning' href='report/export.php?st_rep=$st_rep&cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&cc_matrix=$cc_matrix&cc_m2=$cc_m2&cc_ftm=$cc_ftm&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region&skala=$skala&depart=$departemen_skala&unitSkala=$unit_skala&tipe_report=$tipe_report&channel=$select_channel'>export to excel</a>";
		}else{
...
if((isset($searchButton) and $jum_tot<>"0" and $report_by<>"0" and ($date_eva1<>"" and $date_eva2<>"")) or ($kd_del=="ok"and $jum_tot<>"0")){
  if ($st_rep =='2'){ //menampilkan data baru
  ...
  <table width="536%" border="1" align="center" cellspacing="1" id="dyntable" class="table table-bordered table-infinite">
    <thead>
      <tr bgcolor="#CCCCCC">
		...
		<th width="1%" rowspan="3" class="head0"><div align="center">MSISDN </div></th>
	 ...
	 <? if ($SES_KODE_LEVEL=="admin" or $SES_KODE_LEVEL=="mitra" or $SES_KODE_LEVEL=="validator"){?>
	  ...
	  <? } ?>
        ...
        <th width="2%" rowspan="3" class="head0"><div align="center">Channel</div></th>
		<th width="2%" rowspan="3" class="head0"><div align="center">Privacy Policy</div></th>
      </tr>
      <tr bgcolor="#CCCCCC">
...
//echo "$q1_yes_jum";
	...
	// if((isset($searchButton) and $jum_tot<>"0") or ($kd_del=="ok"and $jum_tot<>"0"))
					...
					//var_dump($skala);
					$q_user="[sp_search3]'table_qm','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$c_observer%','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala','$select_channel'";
					// echo $q_user;
					//echo "456456";
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
					  ...
					  ?>
    <a href="<? echo "del_group.php?report_by=$report_by&kode_del=detail_inbound&id_qm=$id_qm&kd_division=$kd_division&kd_departemen=$kd_departemen&kd_unit=$kd_unit&date_eva1=$date_eva1&date_eva2=$date_eva2&c_observer=$observer"; ?>" onclick="return confirmDelete2();">
    <?
					  	 echo "delete</a></td>";
  ...
  <thead>
    ...
    <tr>
      ...
      <td>
		<?  
		  $queryhendi="[sp_reportqm_inbound2]'table_qm','$tgl1','$tgl2','$report_by','%$c_observer%','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala','','$select_channel'";
    ...
    <tr>
      <td rowspan="5">2</td>
      <td rowspan="5">Interaksi, Konfirmasi & Solusi</td>
      <td>Probing</td>
	  <td>5</td>
      <td>
		<?  
  ...
  <?  
	  		if ($select_region !="") {
	  			if ($report_by=="Unit")
				  	{
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c
					   inner join db_qm.dbo.table_qm a on a.id_qm=c.id_qm 
					   inner join cco.hrms.dbo.tb_data_pribadi b on a.id_pribadi_user=b.id_data_pribadi
					   where b.region ='$select_region' and a.status_save='complete' and repeat='yes' and a.channel='$select_channel' 
	  			...
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c
					   inner join db_qm.dbo.table_qm a on a.id_qm=c.id_qm 
					   inner join cco.hrms.dbo.tb_data_pribadi b on a.id_pribadi_user=b.id_data_pribadi
					   where a.status_save='complete' and repeat='yes' and a.channel='$select_channel' 
					   and a.id_unit like '%$kd_unit%' and  convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2'";
	   				}
	  			if ($report_by=="Departement")
				  	{	
	  			...
	  			if ($report_by=="Observer")
				  	{
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c inner join db_qm.dbo.table_qm a
					  			on a.id_qm=c.id_qm inner join cco.hrms.dbo.tb_data_pribadi z  on a.id_pribadi_observer=cast(id_data_pribadi as varchar)
					  			where a.status_save='complete' and repeat='yes' and  cast(id_data_pribadi as varchar)='$c_observer' and a.channel='$select_channel' 
					  			and  convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2'";  
					}
	  			...
	  			if ($report_by=="Agent")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c inner join db_qm.dbo.table_qm a
					  			...
					  			where a.status_save='complete' and repeat='yes' and  (z.nik_karyawan like '%$nik_agent%' or z.nama like '%$nik_agent%') and a.channel='$select_channel' 
					  			and  convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2'";   
					}
					if ($report_by=="Skala")
				  	{	
...
case when sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12)
 >= 90 then 'A' when sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12)
 >= 80 and sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12)
 < 90 then 'B' when sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12)
 >= 70 and sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12)
 < 80 then 'C' when sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12)
 >= 60 and sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12)
 < 70 then 'D' when sum ( save_q1+save_q2+save_q3+save_q4+save_q5+save_q6+save_q7+save_q8+save_q9+save_q10+save_q11+save_q12) 
< 60 then 'E' end 'Skala' from db_qm.dbo.table_qm a with(nolock) 
inner join db_qm.dbo.table_qm_detail b with(nolock) on a.id_qm=b.id_qm
 where a.status_save='complete' and convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2' 
 and a.id_unit like '%$departemen_skala%' and a.id_unit like '%$unit_skala%' and a.channel='$select_channel' 
	...
	<tr>
      ...
      <td width="68">
		<?  
			...
			$has=(($jum_tot-$rep_tot_y-$q7_tot_n)/$jum_tot)*100;
			//echo "rep : $rep_tot_y";
			printf("%1.2f ", $has);
		?>
	  </td>
...
<h4 class="widgettitle">Chart Inbound</h4>
...
<script type="text/javascript">
    Highcharts.chart('containerdc', {
        ...
        xAxis: {
            categories: [
                'Greeting with Antusiasm',
                'Verify (refer to verification rules)',
                'Probing',
                'Correct Answer / Solution & Process Complience',
                'Input Accuracy Data',
                'Information/ Notes',
                'FCR',
	...
	if($tipe_report=='summary'){
		...
		$a_user="[new_report_email]'table_qm','$report_by','$tgl1','$tgl2','$select_region','$st_rep','summary','','$select_channel' ";
		...
		while($c_user=mssql_fetch_array($b_user)){
			echo "<tr class='content'>
				...
				<td>
				<?php if($c_user['noe']!=0){ ?>
					<a href="report/report_inbound_show.php?id_pribadi_user=<?=$c_user['id_pribadi_user']?>&report_by=<?=$report_by?>&kd_unit=<?=$kd_unit?>&tgl1=<?=$tgl1?>&tgl2=<?=$tgl2?>&cc_mentari=<?=$cc_mentari?>&cc_im3=<?=$cc_im3?>&c_observer=<?=$c_observer?>&nik_agent=<?=$nik_agent?>&select_region=<?=$select_region?>&skalacmb=<?=$skalacmb?>&departemen_skala=<?=$departemen_skala?>&unit_skala=<?=$unit_skala?>&st_rep=<?=$st_rep?>&channel=<?=$select_channel?>" onClick="return popup2(this, 'notes');"><?=(string)$c_user['noe']?></a>
				<?php }else{
						echo $c_user['noe'];
					}
				?>
		...
		$a_user="[new_report_email]'table_qm','$report_by','$tgl1','$tgl2','$select_region','$st_rep','detail','','$select_channel' ";
		// echo"$a_user";
		...
		while($c_user=mssql_fetch_array($b_user)){
			...
			if($c_user['noe']>1 and $tmp!=$c_user['id_pribadi_user']){
				...
				";
			if($c_user['noe']>1 and $tmp!=$c_user['id_pribadi_user']){?>
				<td rowspan="<?php echo $c_user['noe']; ?>"><a href="report/report_inbound_show.php?id_pribadi_user=<?=$c_user['id_pribadi_user']?>&report_by=<?=$report_by?>&kd_unit=<?=$kd_unit?>&tgl1=<?=$tgl1?>&tgl2=<?=$tgl2?>&cc_mentari=<?=$cc_mentari?>&cc_im3=<?=$cc_im3?>&c_observer=<?=$c_observer?>&nik_agent=<?=$nik_agent?>&select_region=<?=$select_region?>&skalacmb=<?=$skalacmb?>&departemen_skala=<?=$departemen_skala?>&unit_skala=<?=$unit_skala?>&st_rep=<?=$st_rep?>&channel=<?=$select_channel?>" onClick="return popup2(this, 'notes');"><?=(string)$c_user['noe']?></a></td>
				<?
			}else if($c_user['noe']>1 and $tmp==$c_user['id_pribadi_user']){?>
				<td style='display:none'></td>
				<?
			}else if($c_user['noe']==1){?>
				<td><a href="report/report_inbound_show.php?id_pribadi_user=<?=$c_user['id_pribadi_user']?>&report_by=<?=$report_by?>&kd_unit=<?=$kd_unit?>&tgl1=<?=$tgl1?>&tgl2=<?=$tgl2?>&cc_mentari=<?=$cc_mentari?>&cc_im3=<?=$cc_im3?>&c_observer=<?=$c_observer?>&nik_agent=<?=$nik_agent?>&select_region=<?=$select_region?>&skalacmb=<?=$skalacmb?>&departemen_skala=<?=$departemen_skala?>&unit_skala=<?=$unit_skala?>&st_rep=<?=$st_rep?>&channel=<?=$select_channel?>" onClick="return popup2(this, 'notes');"><?=(string)$c_user['noe']?></a>
				<?
			}
			echo "</tr>";
