...
header("Content-Disposition: attachment; filename=report_process_level.xls"); 
...
<p>&nbsp;</p>
  <table width="100%" border="1" align="center" cellspacing="1" class="table table-bordered table-infinite">
	...
	include "../konek_qm.php";
	$itung="[sp_count_pl]'$date_eva1','$date_eva2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region'";
	// echo $itung;
...
//echo "$q1_yes_jum";
					...
					//var_dump($skala);
					//$q_user="[sp_search3]'table_qm','$report_by','%$kd_unit%','$date_eva1','$date_eva2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala' ";
					$q_user="[sp_search_pl]'$report_by','%$kd_unit%','$date_eva1','$date_eva2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$select_region','%$c_observer%','%$nik_agent%'";
					// echo $q_user;
					//echo "456456";
...
<table width="436" border="1" class="table table-bordered table-infinite">
  <thead>
    ...
    <tr>
      ...
      <td>
		<?  
		  //$queryhendi="[sp_reportqm_inbound]'table_qm','$date_eva1','$date_eva2','$report_by','$c_observer','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala',''";
		  $query_rpt="[sp_reportqm_pl]'$date_eva1','$date_eva2','$report_by','%$c_observer%','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region'";
	  		...
	  		if ($select_region !="") {
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_pl a
					   ...
					   and a.id_unit like '%$kd_unit%' and  convert(varchar,a.observation_date,112) between '$date_eva1' and '$date_eva2'";
	   				}
	...
	<tr>
      ...
      <td width="68">
		<?  
			...
			$stot_hh="[sp_handling_pl]'$report_by','$date_eva1','$date_eva2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
			//echo "$stot_hh";	
