  ...
  <table width="386" border="1" align="center" class="table table-bordered table-infinite">
    ...
    <tr>
      ...
      <td><? $observation_date=date("m/d/Y");?>
        <span class="content">
        <input name="observation_date" disabled value="<?php echo $observation_date; ?>"  type="text" id="datepicker1"  datepicker="true" datepicker_format="MM/DD/YYYY"  class="input-large" tabindex="2">
      </span></td>
    </tr>
    <tr>
      <td>Week</td>
...
<?
		  ...
		  if ($Submit)
	  {
		...
		if($ss="" or $spv_name=="" or $interaction_date=="" or $observation_date=="" or $week_=="0" or $customer_name=="" or $interaction_type=="0" or $product_type=="" or $product_detail=="" or $product_detail_agent=="" or $fcr=="0" or  hh_2=="" or $mm_2=="" or $notes_recording=="")
			{
			var_dump($observation_date);
			?>
            <script type="text/javascript">
		...
		$ss2=00;
				  ...
				  $s_spv_name = str_replace("'", "", "$spv_name");
				  $s_customer_name = str_replace("'", "", "$customer_name");
				  $s_product_type = str_replace("'", "", "$product_type");
				  $s_product_detail = str_replace("'", "", "$product_detail");
				  $s_product_detail_agent = str_replace("'", "", "$product_detail_agent");	
				  $s_notes_recording = str_replace("'", "", "$notes_recording");
				  
		$interaction_date_save="$interaction_date $hh$slas$mm$slas$ss2"; 
		$q_addTemp1="insert into table_qm_galeri_detail(id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,fcr,hh_handling_time,mm_handling_time,ss_handling_time,notes_recording)
		  values('$id_qm','$s_spv_name','$interaction_date_save',getdate(),'$week_','$s_customer_name','$interaction_type','$s_product_type','$s_product_detail','$s_product_detail_agent','$fcr','$hh_2','$mm_2','$ss_2','$s_notes_recording')";
		$q_addTemp="insert into table_qm_galeri_detail(id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,fcr,hh_handling_time,mm_handling_time,ss_handling_time,notes_recording)
		  values('$id_qm','$s_spv_name','$interaction_date_save','$observation_date','$week_','$s_customer_name','$interaction_type','$s_product_type','$s_product_detail','$s_product_detail_agent','$fcr','$hh_2','$mm_2','$ss_2','$s_notes_recording')";
		...
		$simpan_data=mssql_query($q_addTemp);
		  //echo "$q_addTemp";
			...
			if ($simpan_data)
					 { 
					$q_addTemp2="update table_qm_galeri set status_save='complete' where id_qm='$id_qm'";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_galeri_detail.php"
		</script> 
          <?php
		}
