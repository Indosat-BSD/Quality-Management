...
<?
		  ...
		  if ($Submit)
	  {
		...
		* nama ketika diupload, jika $sameName > 0 maka pakai format "namafile($sameName).ext */
		...
		$q_addTemp="insert into table_qm_inbound_top_detail(strength_agent,improved,recommendation,
		action_done_qa,action_done_ops,file_action_done_qa,file_action_done_ops,id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,repeat,date_prev_call,prev_agent,hh_handling_time,mm_handling_time,ss_handling_time,drop_call,notes_recording)
		  values('$s_strength_agent','$s_improved','$s_recommendation','$s_action_done_qa','$s_action_done_ops','$newName','$newName2','$id_qm','$spv_name','$interaction_date_save',getdate(),'$week_','$customer_name','$interaction_type','$product_type','$product_detail','$product_detail_agent','$repeat','$date_prev_call','$prev_agent','$hh_2','$mm_2','$ss_2','$drop_call','$s_notes_recording')";
		  $simpan_data=mssql_query($q_addTemp);
		  //echo "$q_addTemp";
		   if ($simpan_data)
			{ 
				$q_addTemp2="update table_qm_inbound_top set status_save='complete',date_saved=getdate() where id_qm='$id_qm'";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_inbound_top_detail.php"
		</script> 
          <?php
		}
