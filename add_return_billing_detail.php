...
:
  ...
  (hh:mm:ss) </td>
    ...
    <tr>
      ...
      <td><input name="total_score" type="text" disabled id="total_score" value="<? if($total_score>=98){$tampiltotal=round($total_score,0);}else{$tampiltotal=round($total_score,2);} echo"$tampiltotal"; ?>" size="3" maxlength="3" class="input-large"></td>
    </tr>
	<tr>
...
<?
		  ...
		  if ($Submit)
	  {
		...
		$q="select id_qm_detail from table_qm_rb_detail where id_qm_detail ='$id_qm'";
		$qq=mssql_query($q);
		$qqq=mssql_fetch_array($qq);
		$ada=$qqq["id_qm_detail"];
		...
		if($ada !=''){
			?>
			  <script type="text/javascript">
				...
				//window.close();
				window.alert("Data Sudah Di Submit");
				window.close();
			  </script> 
          <?php		
		}else{
		
		if($spv_name=="" or $interaction_date=="" or $observation_date=="" or $week_=="0" or $customer_name=="" or $interaction_type=="0" or $product_type=="" or $product_detail=="" or $product_detail_agent=="" or $hh_2=="" or $mm_2=="" or $notes_recording=="")
			{
			?>
            <script type="text/javascript">
		...
		* nama ketika diupload, jika $sameName > 0 maka pakai format "namafile($sameName).ext */
		...
		/*/
		$q_addTemp="insert into table_qm_rb_detail(action_done_qa,action_done_ops,file_action_done_qa,file_action_done_ops,id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,call_activity,hh_handling_time,mm_handling_time,ss_handling_time,call_status,notes_recording)
		  values('$s_action_done_qa','$s_action_done_ops','$newName','$newName2','$id_qm','$spv_name','$interaction_date_save','$date_saved','$week_','$customer_name','$interaction_type','$product_type','$product_detail','$product_detail_agent','$call_activity','$hh_2','$mm_2','$ss_2','$call_status','$s_notes_recording')";
			  $simpan_data=mssql_query($q_addTemp);
		  echo "$q_addTemp";
			   if ($simpan_data)
				{ 
				$q_addTemp2="update table_qm_rb set status_save='complete',date_saved='$observation_date' where id_qm='$id_qm'";
				echo "$q_addTemp2";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_return_billing_detail.php"
		</script> 
          <?php
		}
