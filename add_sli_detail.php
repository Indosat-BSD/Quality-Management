...
<?
		  ...
		  if ($Submit)
	  {
		...
		$q_addTemp="insert into table_qm_sli_detail(recommendation,id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,fcr,repeat,date_prev_call,prev_agent,hh_handling_time,mm_handling_time,ss_handling_time,drop_call,notes_recording)
		  values('$s_recommendation','$id_qm','$spv_name','$interaction_date_save',getdate(),'$week_','$customer_name','$interaction_type','$product_type','$product_detail','$product_detail_agent','$fcr','$repeat','$date_prev_call','$prev_agent','$hh_2','$mm_2','$ss_2','$drop_call','$s_notes_recording')";
		  mssql_query($q_addTemp);
		 // echo "$q_addTemp";
		  
		$q_addTemp2="update table_qm_sli set status_save='complete',date_saved=getdate() where id_qm='$id_qm'";
		  $simpan_data=mssql_query($q_addTemp2);
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_sli_detail.php"
		</script> 
          <?php
		}
