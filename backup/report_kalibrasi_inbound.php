...
$r_observerCombo = mssql_query( $q_observerCombo );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='rep_kalibrasi/export_kalibrasi.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
  ...
  <thead>
    <tr bgcolor="#CCCCCC">
      ...
      <th rowspan="2" class="head0"><div align="center">Drop Call</div></th>
	...
	$itung="[sp_count_kalibrasi]'table_kalibrasi','table_qm','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region'";
...
//if ($repeat=="yes")
  ...
  <thead>
    ...
    <tr>
      ...
      <td><?  
	  //edit hendi
      $queryhendi="[sp_reportqm_kalibrasi]'table_kalibrasi','table_qm','$tgl1','$tgl2','$report_by','$c_observer','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region'";
...
printf("%1.2f ", $prosen_q14_na);
  ...
  <table width="264" border="1" class="table table-bordered table-infinite">
    ...
    <tr>
      <td>CE Total </td>
      <td><?  
					$stot_fe="[sp_fetotal_kalibrasi]'table_kalibrasi','table_qm','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
					//echo $stot_fe;
    ...
    <tr>
      ...
      <td><?  
	  		if ($select_region !="") {
	  			if ($report_by=="Unit")
				  	{
				 	  $stot_fcr="select count(*) as jum_s_fcr from db_qm.dbo.table_qm_detail c 
	  			...
	  			if ($report_by=="Validator")
				  	{
				 	 $stot_fcr="select count(*) as jum_s_fcr from db_qm.dbo.table_qm_detail c inner join db_qm.dbo.table_qm a
    ...
    <tr>
      ...
      <td><?  
					$stot_hh="[sp_handling_kalibrasi]'table_kalibrasi','table_qm','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
					//echo $stot_hh;
    ...
    <tr>
      <td>Drop Call (Y) </td>
      <td><?  
	   $stot_d="[sp_dropcall_kalibrasi]'table_kalibrasi','table_qm','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
	   //echo $stot_d;
