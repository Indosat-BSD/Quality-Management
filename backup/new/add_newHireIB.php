...
<form action="<? $PHP_SELF; ?>" method="post" name="welcome" id="welcome" onKeyPress="return noenter()">
  ...
  <table width="313" border="0">
    <tr>
      ...
      <td>
	  <?php
							 echo "<a class='btn btn-leave' href='?index=add_newHireIB&sr=1'>Karyawan</a>";
							 echo "<a class='btn btn-pilih' href='?index=add_newHireIB2&sr=0'>Seleksi</a>";
	  ?>
	  </td>
    </tr>
    <tr>
  ...
  <thead>
    ...
    <?
	$stot="select count(*)as jum_s from db_qm.dbo.table_qm_newHireIB where status_save='not_complete' and id_pribadi_observer='$SES_hendi'";
//echo "$stot";
		...
		if($jum_tot1<>"0")
			...
			
			{
					...
					while($d_user=mssql_fetch_array($r_user))
					  ...
					  <td>$nik</td>";
                     if($akd_departemen == CCO01 or $akd_departemen == CCO03){
                echo"                     
                      <td><a href='?index=add_newHireIB&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$full_name&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi&sr=1'>Create</a>";
						}else{ echo "<td></td>";}
					  echo "</td> </tr>";
					  $no++;
					}
				...
				echo "<tr class='content'>
                      ...
                      <td><a href='?index=add_newHireIB&div=cco&edit=t&userTextBox=$usernameS&namaTextBox=$lb&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi&nu=$nu'>Create</a>";
...
$result = mssql_query( $query );
  ...
  <table width="935" border="0">
    <tr>
		  ...
		  if ($Submit)
	  {
		...
		$q_addTemp="insert into table_qm_newHireIB(q1,q2,q3,q4,q5,q6,q7,q8,q9,q10,q11,q12,save_q1,save_q2,save_q3,save_q4,save_q5,save_q6,save_q7,save_q8,save_q9,save_q10,save_q11,save_q12,save_q13,save_q14,status_save,id_pribadi_user,id_pribadi_observer,date_saved,tot_score,id_unit,lb,notes1,notes2,notes3,notes4,notes5,notes6,notes7,notes8,notes9,notes10,notes11,notes12,flag_status)
					values('$sc1','$sc2','$sc3','$sc4','$sc5','$sc6','$sc7','$sc8','$sc9','$sc10','$sc11','$sc12','$tot1','$tot2','$tot3','$tot4','$tot5','$tot6','$tot7','$tot8','$tot9','$tot10','$tot11','$tot12','0','0','not_complete','$pribadi','$SES_hendi',getdate(),'$save_total','$kd_unit','$id_lb','$s_notes1','$s_notes2','$s_notes3','$s_notes4','$s_notes5','$s_notes6','$s_notes7','$s_notes8','$s_notes9','$s_notes10','$s_notes11','$s_notes12','1')";
		mssql_query($q_addTemp);
		//echo "$q_addTemp";
		  //echo "$q_addTemp";
		  
  echo"<script type='text/javascript'>
		window.location='?index=add_newHireIB&div=cco'
		</script> ";
	  	}
	  }
  ...
  <thead>
    ...
    <?
					/*$q_user="select a.*,b.nama,b.nik_karyawan, case when a.lb !='' then a.lb else b.nama end as lb from db_qm.dbo.table_qm_newHireIB a
inner join hrms.dbo.tb_data_pribadi b on a.id_pribadi_user=cast(b.id_data_pribadi as varchar) where  a.id_pribadi_observer='$SES_hendi' and a.status_save='not_complete'";*/
					$q_user="[sp_User_NotComplete]'table_qm_newHireIB','$SES_hendi'";
					//echo "$q_user";
					$r_user=mssql_query($q_user);
					$no2=1;
					while($d_user=mssql_fetch_array($r_user))
...
echo "<td>";
					  ...
					  <a href="<? echo "del_group.php?kode_del=add_newHireIB2&id_qm=$id_qm"; ?>" onclick="return confirmDelete2();">
					  <?
					  echo "delete</a></td><td>";
					  ?>
