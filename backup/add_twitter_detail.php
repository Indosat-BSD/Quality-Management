...
<?
		  ...
		  if ($Submit)
	  {
		...
		$q_addTemp="insert into table_qm_twitter_detail(recommendation,id_qm,tenure,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,repeat,date_prev_call,prev_agent,notes_recording)
		  values('$s_recommendation','$id_qm','$tenure','$spv_name','$interaction_date_save',getdate(),'$week_','$customer_name','$interaction_type','$product_type','$product_detail','$product_detail_agent','$repeat','$date_prev_call','$prev_agent','$s_notes_recording')";
		  $simpan_data=mssql_query($q_addTemp);
		 // echo "$q_addTemp";
		  if ($simpan_data)
					 { 
		$q_addTemp2="update table_qm_twitter set status_save='complete' where id_qm='$id_qm'";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_twitter_detail.php"
		</script> 
          <?php
		}
