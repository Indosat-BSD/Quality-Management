...
from db_qm.dbo.Table_kalibrasi a inner join
...
$r_observerCombo = mssql_query( $q_observerCombo );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='rep_kalibrasi/export_kalibrasi_pl.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
  ...
  <thead>
	...
	//$itung="[sp_count_kalibrasi]'table_kalibrasi','table_qm_pl','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
	 $itung="[sp_count_kalibrasi_pl]'$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
...
//echo "$q1_yes_jum";
	...
	if((isset($searchButton) and $jum_tot<>"0") or ($kd_del=="ok"and $jum_tot<>"0"))
					...
					//$q_user="[sp_search_kalibrasi]'table_kalibrasi','table_qm','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
					$q_user="[sp_search_kalibrasi_pl]'$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$select_region','$c_observer','%$nik_agent%'";
					//echo $q_user;
					//echo "456456";
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
					  ...
					  <a href="<? echo "del_kalibrasi.php?report_by=$report_by&kode_del=detail_pl&id_qm=$id_qm&kd_division=$kd_division&kd_departemen=$kd_departemen&kd_unit=$kd_unit&date_eva1=$date_eva1&date_eva2=$date_eva2&c_observer=$observer&id=$id"; ?>" onclick="return confirmDelete2();">
					  <?
					  	 echo "delete</a></td>";
				...
				<a href="<? echo "edit_kalibrasi/edit_kalibrasi_pl.php?report_by=$report_by&date_eva1=$date_eva1&date_eva2=$date_eva2&id_qm=$id_qm&msisdn=$msisdn&customer_name=$customer_name&observer_name=$observname&interaction_date=$interaction_date&hh_duration=$hh_dur&hh_it=$hh_time&mm_it=$mm_time&mm_duration=$mm_dur&ss_duration=$ss_dur&observation_date=$observation_date&week_=$week_&interaction_type=$interaction_type&product_knowledge=$product_knowledge&notes_pk=$notes_pk&activity_code=$activity_code&notes_ac=$notes_ac&handling_process=$handling_process&fcr=$fcr&notes_fcr=$notes_fcr&repeat=$repeat&notes_repeat=$notes_repeat&anthusiasm=$anthusiasm&notes_anthusiasm=$notes_antusiasm&manner=$manner&notes_manner=$notes_manner&effective_handling=$effective_handling&notes_eh=$notes_eh&observer=$observer&kd_unit=$kd_unit&id_pribadi_user=$id_pribadi_user&notes_score=$notes_score&agree=$agree"; ?>" onClick="return popup(this, 'notes');">
					  <?
					  	 echo "Edit </a></td>";
						 }		  
...
//if ($repeat=="yes")
  ...
  <thead>
    ...
    <tr>
      ...
      <td><?  
	  //edit hendi
      $queryhendi="[sp_reportqm_kalibrasi_pl]'$tgl1','$tgl2','$report_by','$c_observer','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region'";
...
if ($select_region !="") {
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_kalibrasi_pl a
					   ...
					   and a.kd_unit like '%$kd_unit%' and  convert(varchar,a.observation_date,112) between '$tgl1' and '$tgl2'";
	   				}
	...
	<tr>
      ...
      <td width="68">
		<?  
			...
			$stot_hh="[sp_handling_kalibrasi_pl]'$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
			//echo "$stot_hh";	
