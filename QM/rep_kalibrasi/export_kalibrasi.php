...
header("Content-Disposition: attachment; filename=report_kalibrasi_{$channel}.xls"); 
...
<table width="368%" border="1" align="center" cellspacing="1">
  ...
  <?
	...
	$itung="[sp_count_kalibrasi]'table_kalibrasi','table_qm','$date_eva1','$date_eva2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$channel'";
	 $queryitung=mssql_query($itung);
	 $hasilitung=mssql_fetch_array($queryitung);
	 $jum_tot=$hasilitung["jum_s"];
//echo "$q1_yes_jum";
	  	
					$q_user="[sp_search_kalibrasi]'table_kalibrasi','table_qm','$report_by','%$kd_unit%','$date_eva1','$date_eva2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$channel'";
					//echo"$q_user";
...
<table width="568" border="1">
  <tr bgcolor="#CCCCCC">
    ...
    <tr>
      ...
      <td><?  
	  //edit hendi
      $queryhendi="[sp_reportqm_kalibrasi_inbound2]'table_kalibrasi','table_qm','$date_eva1','$date_eva2','$report_by','$c_observer','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$channel'";
...
inner join db_qm.dbo.table_qm_detail b with(nolock) on a.id_qm=b.id_qm
 where a.status_save='complete' and convert(varchar,a.date_saved,112)  and a.channel='$channel' 
