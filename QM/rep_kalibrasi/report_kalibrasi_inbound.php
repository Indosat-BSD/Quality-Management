...
$r_observerCombo = mssql_query( $q_observerCombo );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='rep_kalibrasi/export_kalibrasi.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region&channel=$select_channel'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
  ...
  <thead>
    <tr bgcolor="#CCCCCC">
      ...
      <th width="1%" rowspan="3" class="head0"><div align="center">Unit Kerja</div></th>
		...
		<th width="4%" rowspan="3" class="head0">Notes Sample Recording</th>
	...
	$itung="[sp_count_kalibrasi]'table_kalibrasi','table_qm','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$select_channel'";
...
//echo "$q1_yes_jum";
	...
	if((isset($searchButton) and $jum_tot<>"0") or ($kd_del=="ok"and $jum_tot<>"0"))
					...
					$q_user="[sp_search_kalibrasi]'table_kalibrasi','table_qm','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$select_channel'";
					//echo $q_user;
					//echo "456456";
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
					  ...
					  <a href="<? echo "del_kalibrasi.php?report_by=$report_by&kode_del=detail_inbound&id_qm=$id_qm&kd_division=$kd_division&kd_departemen=$kd_departemen&kd_unit=$kd_unit&date_eva1=$date_eva1&date_eva2=$date_eva2&c_observer=$observer&id=$id"; ?>" onclick="return confirmDelete2();">
					  <?
					  	 echo "delete</a></td>";
...
//if ($repeat=="yes")
  ...
  <thead>
    ...
    <tr>
      ...
      <td><?  
	  //edit hendi
      $queryhendi="[sp_reportqm_kalibrasi_inbound2]'table_kalibrasi','table_qm','$tgl1','$tgl2','$report_by','$c_observer','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$select_channel'";
		...
		<?  
	  		if ($select_region !="") {
	  			...
	  			if ($report_by=="Validator")
				  	{
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c inner join db_qm.dbo.table_qm a
					  			on a.id_qm=c.id_qm inner join cco.hrms.dbo.tb_data_pribadi z  on a.id_kalibrasi=cast(id_data_pribadi as varchar)
...
inner join db_qm.dbo.table_qm_detail b with(nolock) on a.id_qm=b.id_qm
 where a.status_save='complete' and convert(varchar,a.date_saved,112) and a.channel='$select_channel' 
