...
if (typeof(mylink) == 'string')
   ...
   href=mylink.href;
window.open(href, windowname, 'width=600,height=100,scrollbars=yes');
return false;
}
...
<form action="<? $PHP_SELF; ?>" method="post" name="welcome" id="welcome" onKeyPress="return noenter()">
  ...
  <thead>
    ...
    <?
	$stot="select count(*)as jum_s from db_qm.dbo.table_qm_credit where status_save='not_complete' and id_pribadi_observer='$SES_hendi'";
//echo "$stot";
		...
		if($jum_tot1<>"0")
			...
			
			{
					...
					while($d_user=mssql_fetch_array($r_user))
                     ...
                     // if($akd_unit == 'CCC02C'){
                echo"                       
                      <td><a href='?index=add_credit_control&div=ccc&edit=t&userTextBox=$usernameS&namaTextBox=$full_name&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi'>Create</a>";
						// }else{ echo "<td></td>";}
					  echo "</td> </tr>";
					  $no++;
					}
				...
				echo "<tr class='content'>
                      ...
                      <td><a href='?index=add_credit_control&div=ccc&edit=t&userTextBox=$usernameS&namaTextBox=$lb&id_lb=$lb&searchButton=1&kd_division=$akd_divisi&kd_departemen=$akd_departemen&userid=$userid&userText=$userText&namaText=$namaText&kd_unit=$akd_unit&useridHidden=$userid&nikTextBox=$nik&pribadi=$pribadi&nu=$nu'>Create</a>";
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
		  ...
		  if ($Submit)
	  {
		...
		if($userTextBox=="" or $namaTextBox=="" or $kd_division=="0" or $kd_departemen=="0" or $kd_unit=="0" or $sc1=="" or $sc2==""  or $sc3=="" or $sc4=="" or $sc5=="" or $sc6=="" or $sc7=="" or $sc8=="" or $sc9=="" or $sc10==""  or $sc11=="" or $sc12=="")
			{
			?>
            <script type="text/javascript">
		...
		$q_addTemp="insert into table_qm_credit(notes1,notes2,notes3,notes4,notes5,notes6,notes7,notes8,notes9,notes10,notes11,notes12,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10,q11,q12,save_q1,save_q2,save_q3,save_q4,save_q5,save_q6,save_q7,save_q8,save_q9,save_q10,save_q11,save_q12,status_save,id_pribadi_user,id_pribadi_observer,date_saved,tot_score,id_unit,lb)
		  values('$s_notes1','$s_notes2','$s_notes3','$s_notes4','$s_notes5','$s_notes6','$s_notes7','$s_notes8','$s_notes9','$s_notes10','$s_notes11','$s_notes12','$sc1','$sc2','$sc3','$sc4','$sc5','$sc6','$sc7','$sc8','$sc9','$sc10','$sc11','$sc12','$tot1','$tot2','$tot3','$tot4','$tot5','$tot6','$tot7','$tot8','$tot9','$tot10','$tot11','$tot12','not_complete','$pribadi','$SES_hendi',getdate(),'$save_total','$kd_unit','$id_lb')";
		 mssql_query($q_addTemp);
		  //echo "$q_addTemp";
		  
  echo"<script type='text/javascript'>
		window.location='?index=add_credit_control&div=ccc'
	</script> ";
	  	}
	  }
  ...
  <thead>
    <tr bgcolor="#CCCCCC">
      ...
      <th width="4%" rowspan="3" class="head0"><div align="center">NIK</div></th>
	  <th width="4%" rowspan="3" class="head0"><div align="center">Username</div></th>
	   ...
	   <th width="4%" class="head0" rowspan="3"><div align="center">Total Score </div></th>
      <th width="4%"  class="head0" rowspan="3"><div align="center">Skala</div></th>
      <th width="4%" colspan="2" class="head0" rowspan="3"><div align="center">Observer</div></th>
	</tr>
    ...
    <?
					/*$q_user="select a.*,b.nama,b.nik_karyawan, case when a.lb !='' then a.lb else b.nama end as lb from db_qm.dbo.table_qm_credit a
inner join cco.hrms.dbo.tb_data_pribadi b on a.id_pribadi_user=cast(b.id_data_pribadi as varchar) where  a.id_pribadi_observer='$SES_hendi' and a.status_save='not_complete'";*/
					$q_user="[sp_User_NotComplete]'table_qm_credit','$SES_hendi'";
					//echo "$q_user";
					$r_user=mssql_query($q_user);
					$no2=1;
					while($d_user=mssql_fetch_array($r_user))
					...
					while($d_user1=mssql_fetch_array($r_user1))
					  ...
					  ?>
					  <a href="<? echo "add_credit_detail.php?observer_name=$observer_name&id_qm=$id_qm&full_name=$full_name&observer=$observer&sc_fe=$sc_fe&sc_nfe=$sc_nfe&total_score=$total_score&skala=$skala"; ?>" onClick="return popup(this, 'notes')">
					  <?
