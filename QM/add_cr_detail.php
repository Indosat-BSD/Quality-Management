...
<body>
  ...
  <table width="386" border="1" align="center" class="table table-bordered table-infinite">
    ...
    <tr>
      <td>Product Type</td>
      <td><input name="product_type" type="text" 
	  id="product_type" value="<? echo"$product_type"; ?>"
	  class="input-large" maxlength="200"></td>
    </tr>
    <tr>
    ...
    <tr>
      <td>Call Activity </td>
      <td>
	  <select name="call_activity" id="call_activity" class="uniformselect" onchange="satu.submit()">
        <option value="0">Select
        <?php
...
:
  ...
  :
  <input name="ss_2" type="text" id="mm_222" value="<? echo"$ss_2"; ?>" size="2" maxlength="2" class="input-small" disabled>
  (hh:mm:ss)
	  <?
	  }
	  else
...
:
  ...
  :
  <input name="ss_2" type="text" id="mm_222" value="<? echo"$ss_2"; ?>" size="2" maxlength="2" class="input-small">
  (hh:mm:ss)	
	  <?}?>
	  </td>
    </tr>
    <tr>
      <td>Call Status </td>
      <td>
	  <select name="call_status" id="select9" class="uniformselect"  onchange="satu.submit()">
        <option value="0">Select
        <?php
		$katahh4=array('Answered','No Answered');
		//$katahh4=array('Answered','No Pick Up','Mailbox','Rejected');
...
if ($Submit)
{
		if($recommendation=="" or $spv_name=="" or $interaction_date=="" or $observation_date=="" or $week_=="0" or $customer_name=="" or $interaction_type=="0" or $product_type=="" or $product_detail=="" or $product_detail_agent=="" or $call_activity=="0" or $shh_2=="" or $smm_2=="" or $sss_2=="" or $call_status=="0" or $sr=="" or $notes_recording=="" or $reason=="0")
			{
			?>
            <script type="text/javascript">
		...
		$q_addTemp="insert into table_qm_cr_detail(recommendation,sr,id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,call_activity,hh_handling_time,mm_handling_time,ss_handling_time,call_status,notes_recording,reason,msisdn)
		  values('$s_recommendation','$sr','$id_qm','$spv_name','$interaction_date_save',getdate(),'$week_','$customer_name','$interaction_type','$product_type','$product_detail','$product_detail_agent','$call_activity','$hh_2','$mm_2','$ss_2','$call_status','$s_notes_recording','$reason','$msisdn')";
		 $simpan_data=mssql_query($q_addTemp) ;
		 
		 if ($simpan_data)
		 {
				$q_addTemp2="update table_qm_cr set status_save='complete',date_saved=getdate() where id_qm='$id_qm'";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_cr_detail.php"
		</script> 
          <?php
		}
