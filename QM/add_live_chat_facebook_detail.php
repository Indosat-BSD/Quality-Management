...
<?
		...
		* nama ketika diupload, jika $sameName > 0 maka pakai format "namafile($sameName).ext */
		...
		$q_addTemp="insert into table_qm_lcfb_detail(recommendation,id_qm,tenure,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,repeat,date_prev_call,prev_agent,notes_recording,action_done_qa,action_done_ops,file_action_done_qa,file_action_done_ops,msisdn)
		 ...
		 // echo "$q_addTemp";
		  if ($simpan_data)
					 { 
		$q_addTemp2="update table_qm_lcfb set status_save='complete' where id_qm='$id_qm'";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_live_chat_facebook_detail.php"
		</script> 
          <?php
		}
