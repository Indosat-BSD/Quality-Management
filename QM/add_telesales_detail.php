...
<?
		  ...
		  if ($Submit)
	  {
		...
		if($ss="" or $spv_name=="" or $interaction_date=="" or $observation_date=="" or $week_=="0" or $customer_name=="" or $interaction_type=="0" or $product_type=="" or $product_detail=="" or $product_detail_agent=="" or $call_activity=="0" or hh_2=="" or $mm_2=="" or $call_status=="0" or $notes_recording=="")
			{
			?>
            <script type="text/javascript">
		...
		$q_addTemp="insert into table_qm_telesales_detail(id_qm,spv_name,interaction_date,observation_date,week_,customer_name,interaction_type,product_type,product_detail,product_detail_agent,call_activity,hh_handling_time,mm_handling_time,ss_handling_time,call_status,notes_recording)
		  values('$id_qm','$spv_name','$interaction_date_save',getdate(),'$week_','$customer_name','$interaction_type','$product_type','$product_detail','$product_detail_agent','$call_activity','$hh_2','$mm_2','$ss_2','$call_status','$s_notes_recording')";
		 $simpan_data=mssql_query($q_addTemp);
		 //echo "$q_addTemp";
		   if ($simpan_data)
					 { 
		$q_addTemp2="update table_qm_telesales set status_save='complete',date_saved=getdate() where id_qm='$id_qm'";
	 ...
	 if ($cancel)
		...
		?>
          <script type="text/javascript">
		window.location="add_telesales_detail.php"
		</script> 
          <?php
		}
