...
$result11 = mssql_query( $query11 );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='report/export_credit_controlHUA.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region&skala=$skala&depart=$departemen_skala&unitSkala=$unit_skala&tipe_report=$tipe_report'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
...
<h4 class="widgettitle">Chart Verifikasi HQ</h4>
...
<script type="text/javascript">
    Highcharts.chart('containerdc', {
        ...
        xAxis: {
            categories: [
                'Validation Data & Approval Verification',
                'Change Status SR',
                'Create SR Up Credit Monitoring',
                'Change Credit Monitoring',
                'Aktivasi IDD',
                'Notes',
                'Greeting with enthusiasm',
                'Permission & Purpose to call',
                'Reconfirm the Agreement & Closing Greeting',
                'Hospitality / kesopanan (relate to Manner)',
...
<!--HEADER-->
	...
	if($tipe_report=='summary'){
		...
		$a_user="[new_report_CC]'table_qm_credit','$report_by','$tgl1','$tgl2','$select_region','summary' ";
		...
		$a_user="[new_report_CC]'table_qm_credit','$report_by','$tgl1','$tgl2','$select_region','detail'";
		...
		while($c_user=mssql_fetch_array($b_user)){
			...
			if($c_user['noe']>1 and $tmp!=$c_user['id_pribadi_user']){
				...
				";
			if($c_user['noe']>1 and $tmp!=$c_user['id_pribadi_user']){?>
				<td rowspan="<?php echo $c_user['noe']; ?>"><a href="report/report_credit_show.php?id_pribadi_user=<?=$c_user['id_pribadi_user']?>&report_by=<?=$report_by?>&kd_unit=<?=$kd_unit?>&tgl1=<?=$tgl1?>&tgl2=<?=$tgl2?>&c_observer=<?=$c_observer?>&nik_agent=<?=$nik_agent?>&select_region=<?=$select_region?>&skalacmb=<?=$skalacmb?>&departemen_skala=<?=$departemen_skala?>&unit_skala=<?=$unit_skala?>&st_rep=<?=$st_rep?>" onClick="return popup2(this, 'notes');"><?=(string)$c_user['noe']?></a></td>
				<?
			}else if($c_user['noe']>1 and $tmp==$c_user['id_pribadi_user']){?>
				<td style='display:none'></td>
				<?
			}else if($c_user['noe']==1){?>
				<td><a href="report/report_credit_show.php?id_pribadi_user=<?=$c_user['id_pribadi_user']?>&report_by=<?=$report_by?>&kd_unit=<?=$kd_unit?>&tgl1=<?=$tgl1?>&tgl2=<?=$tgl2?>&c_observer=<?=$c_observer?>&nik_agent=<?=$nik_agent?>&select_region=<?=$select_region?>&skalacmb=<?=$skalacmb?>&departemen_skala=<?=$departemen_skala?>&unit_skala=<?=$unit_skala?>&st_rep=<?=$st_rep?>" onClick="return popup2(this, 'notes');"><?=(string)$c_user['noe']?></a>
				<?
			}
			echo "</tr>";
