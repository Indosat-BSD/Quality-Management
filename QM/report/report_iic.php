...
$result11 = mssql_query( $query11 );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='report/export_iic.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&cc_matrix=$cc_matrix&cc_m2=$cc_m2&cc_ftm=$cc_ftm&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region&skala=$skala_cmb&depart=$departemen_skala&unitSkala=$unit_skala'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
	...
	if ($date_eva1 != "" && $date_eva2 != "") {	
	 ...
	 $itung="[sp_count3]'table_qm_iic','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
	 //$itung="[sp_count3]'table_qm_iic','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala_cmb','$departemen_skala','$unit_skala'";
...
//echo "$q1_yes_jum";
  ...
  <thead>
    <tr bgcolor="#CCCCCC">
	  ...
	  <? if ($SES_KODE_LEVEL<>"mitra" and $SES_KODE_LEVEL<>"iic"){?>
   ...
   <tr bgcolor="#CCCCCC">
	...
	<?					
					$q_user="[sp_search3]'table_qm_iic','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$c_observer%','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala' ";
					//echo"$q_user";
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
					  ...
					  <a href="<? echo "del_group.php?report_by=$report_by&kode_del=detail_iic&id_qm=$id_qm&kd_division=$kd_division&kd_departemen=$kd_departemen&kd_unit=$kd_unit&date_eva1=$date_eva1&date_eva2=$date_eva2&c_observer=$observer"; ?>" onclick="return confirmDelete2();">
					  <?
					  	 echo "delete</a></td>";
					...
					$observname=$qqq["observer_name"];
					  ...
					  <a href="<? echo "edit_iic.php?kode_edit=edit_iic&report_by=$report_by&date_eva1=$tgl1&date_eva2=$tgl2&id_qm=$id_qm&full_name=$full_name&spv_name=$spv_name&observer_name=$observname&interaction_date=$interaction_date2&hh=$hh&mm=$mm&observation_date=$observ&week_=$week_&customer_name=$customer_name&interaction_type=$interaction_type&product_type=$product_type&product_detail=$product_detail&product_detail_agent=$product_detail_agent&fcr=$fcr&repeat=$repeat&date_prev_call=$date_prev_call2&prev_agent=$prev_agent&hh_2=$hh_2&mm_2=$mm_2&ss_2=$ss_2&drop_call=$drop_call&notes1=$notes1&notes2=$notes2&notes3=$notes3&notes4=$notes4&notes5=$notes5&notes6=$notes6&notes7=$notes7&notes8=$notes8&notes9=$notes9&notes10=$notes10&notes11=$notes11&sc1=$q1&sc2=$q2&sc3=$q3&sc4=$q4&sc5=$q5&sc6=$q6&sc7=$q7&sc8=$q8&sc9=$q9&sc10=$q10&sc11=$q11&save_q1=$save_q1&save_q2=$save_q2&save_q3=$save_q3&save_q4=$save_q4&save_q5=$save_q5&save_q6=$save_q6&save_q7=$save_q7&save_q8=$save_q8&save_q9=$save_q9&save_q10=$save_q10&save_q11=$save_q11&userid=$userid&observer=$SES__NIK&date_saved=$date_saved&fe_acc=$sc_fe&nfe_acc=$sc_nfe&total_score=$total_score&skala=$skala&notes_recording=$notes_recording"; ?>" onClick="return popup(this, 'notes');">
					  <?
					  	 echo "Edit</a></td>";
...
?>
					...
					<td><a href="showKalibrasi/show_iic.php?id=<? echo "$id";?>&id_qm=<? echo "$id_qm";?>&agree=<? echo "$agree";?>" onClick="return popup(this, 'notes');">	
					<? if ($id_kalibrasi != ""){
  ...
  <thead>
    ...
    <!--<tr>
      ...
      <td><?  
	  //edit hendi
      /*$queryhendi="[sp_reportqm3]'table_qm_iic','$tgl1','$tgl2','$report_by','$c_observer','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala_cmb','$departemen_skala','$unit_skala'";
...
printf("%1.2f ", $prosen_q1_na);
	...
	<tr>
      ...
      <td>
		<?  
		  $queryhendi="[sp_reportqm_iic]'table_qm_iic','$tgl1','$tgl2','$report_by','%$c_observer%','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala',''";
		  $proses=mssql_query($queryhendi);
		  $result=mssql_fetch_array($proses);
    ...
    <tr style="border:1px solid #CCCCCC;">
      ...
      <td><? 
	  			...
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="[sp_repeat_tot]'table_qm_iic','$report_by','$tgl1','$tgl2','%$kd_unit%','$select_region','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','%$skala%','%$departemen_skala%','%$unit_skala%'";
...
";  */ 
    ...
    <tr style="border:1px solid #CCCCCC;">
      ...
      <td><?  
			...
			$stot_hh="[sp_handling3]'table_qm_iic','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
					//echo $stot_hh;
    ...
    <tr style="border:1px solid #CCCCCC;">
      <td>Drop Call (Y) </td>
      <td><?  
		$stot_d="[sp_dropcall3]'table_qm_iic','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
