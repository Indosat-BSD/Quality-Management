...
header("Content-Disposition: attachment; filename=report_{$channel}.xls"); 
...
<table width="368%" border="1" align="center" cellspacing="1">
  ...
  <?
	...
	$itung="[sp_count3]'table_qm','$date_eva1','$date_eva2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region','$skala','$depart','$unitSkala','$channel'";
...
//echo "$q1_yes_jum";
					...
					$q_user="[sp_search3]'table_qm','$report_by','%$kd_unit%','$date_eva1','$date_eva2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$c_observer%','%$nik_agent%','$select_region','$skala','$depart','$unitSkala','$channel'";
					// echo"$q_user";
 ...
 <table width="700" border="1" class="table table-bordered table-infinite">
  <thead>
    ...
    <tr>
      ...
      <td>
		<?  
		  $queryhendi="[sp_reportqm_inbound2]'table_qm','$date_eva1','$date_eva2','$report_by','%$c_observer%','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala','','$channel'";
	  		...
	  		if ($select_region !="") {
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c
					   ...
					   where b.region ='$select_region' and a.status_save='complete' and repeat='yes' and a.channel='$channel' 
	  			...
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c
					   inner join db_qm.dbo.table_qm a on a.id_qm=c.id_qm 
					   inner join cco.hrms.dbo.tb_data_pribadi b on a.id_pribadi_user=b.id_data_pribadi
					   where a.status_save='complete' and repeat='yes'  and a.channel='$channel' 
					   and a.id_unit like '%$kd_unit%' and  convert(varchar,a.date_saved,112) between '$date_eva1' and '$date_eva2'";
	   				}
	  			if ($report_by=="Departement")
				  	{	
	  			...
	  			if ($report_by=="Observer")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c inner join db_qm.dbo.table_qm a
					  			...
					  			where a.status_save='complete' and repeat='yes' and  cast(id_data_pribadi as varchar)='$c_observer' and a.channel='$channel' 
					  			and  convert(varchar,a.date_saved,112) between '$date_eva1' and '$date_eva2'";  
					}
	  			...
	  			if ($report_by=="Agent")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_detail c inner join db_qm.dbo.table_qm a
					  			...
					  			where a.status_save='complete' and repeat='yes' and  (z.nik_karyawan like '%$nik_agent%' or z.nama like '%$nik_agent%') and a.channel='$channel' 
					  			and  convert(varchar,a.date_saved,112) between '$date_eva1' and '$date_eva2'";   
					}
					if ($report_by=="Skala")
				  	{	
...
inner join db_qm.dbo.table_qm_detail b with(nolock) on a.id_qm=b.id_qm
 where a.status_save='complete' and convert(varchar,a.date_saved,112) between '$date_eva1' and '$date_eva2' and a.channel='$channel'and a.id_unit like '%$departemen_skala%' and a.id_unit like '%$unit_skala%'
...
";   
	...
	<tr>
      ...
      <td width="68">
		<?  
			$stot_hh="[sp_handling3]'table_qm','$report_by','$date_eva1','$date_eva2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala','$channel'";
...
?>
	...
	if($tipe_report=='summary'){
		...
		$a_user="[new_report_email]'table_qm','$report_by','$date_eva1','$date_eva2','$select_region','$st_rep','summary','','$channel' ";
		...
		$a_user="[new_report_email]'table_qm','$report_by','$date_eva1','$date_eva2','$select_region','$st_rep','detail','','$channel' ";
