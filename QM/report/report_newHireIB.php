...
<form action="<? $PHP_SELF ?>" method="post" name="satu" enctype="multipart/form-data" onKeyPress="return noenter()">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
     <tr class="content">
		<td width="20%">
	   					<?php
							 echo "<a class='btn btn-pilih' href='?index=report_newHireIB'>Karyawan</a>";
							 echo "<a class='btn btn-leave' href='?index=report_newHireIB2'>Seleksi</a>";
						?></td>
		<td width="80%">&nbsp;</td>
	</tr>
	<tr class="content">
...
from db_qm.dbo.table_qm_newHireIB a inner join
...
select id_qm from (
	...
	$itung="[sp_count3]'table_qm_newHireIB','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
...
//echo "$q1_yes_jum";
	...
	if((isset($searchButton) and $jum_tot<>"0") or ($kd_del=="ok"and $jum_tot<>"0"))
					...
					//var_dump($skala);
					$q_user="[sp_search3]'table_qm_newHireIB','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$c_observer%','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala' ";
					// echo $q_user;
					//echo "456456";
...
function confirmDelete2()
{
    return confirm('Are you sure you wish to delete this entry?');
					  ...
					  ?>
    <a href="<? echo "del_group.php?report_by=$report_by&kode_del=detail_newHireIB&id_qm=$id_qm&kd_division=$kd_division&kd_departemen=$kd_departemen&kd_unit=$kd_unit&date_eva1=$date_eva1&date_eva2=$date_eva2&c_observer=$observer"; ?>" onclick="return confirmDelete2();">
    <?
					  	 echo "delete</a></td>";
    ...
    <?
			...
			<td><a href="showKalibrasi/show_newHireIB.php?id=<? echo "$id";?>&id_qm=<? echo "$id_qm";?>&agree=<? echo "$agree";?>" onClick="return popup(this, 'notes');">	
			<? if ($id_kalibrasi != ""){
  ...
  <thead>
    ...
    <tr>
      ...
      <td><?  
	  //edit hendi
      $queryhendi="[sp_reportqm_inbound]'table_qm_newHireIB','$tgl1','$tgl2','$report_by','%$c_observer%','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala',''";
  ...
  <?  
	  		if ($select_region !="") {
	  			if ($report_by=="Unit")
				  	{
			 		  $stot_fcr="[sp_fcr_tot]'table_qm_newHireIB','$report_by','$tgl1','$tgl2','%$kd_unit%','$select_region','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','%$skala%','%$departemen_skala%','%$unit_skala%'";
					 ...
					 /* $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_newHireIB_detail c
					   ...
					   $stot_rep="[sp_repeat_tot]'table_qm_newHireIB','$report_by','$tgl1','$tgl2','%$kd_unit%','$select_region','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','%$skala%','%$departemen_skala%','%$unit_skala%'";
	   				}
...
";   */
    ...
    <tr>
      ...
      <td width="68">
		<?  
					$stot_hh="[sp_handling3]'table_qm_newHireIB','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
