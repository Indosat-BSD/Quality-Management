    ...
    <tr>
	  ...
	  ?>
		...
		<input name="cek_score" type="submit" id="cek_score" value="Cek Score"  class="btn"/>
		 <? if($save_total >=98){$scrr=round($save_total,0);}else{$scrr=round($save_total,2);}echo"$scrr";?><br /><br />
		<br />
...
<?
		  ...
		  if ($Submit)
	  {
		...
		$interaction_date_save="$interaction_date $hh$slas_e$mm$slas_e$ss2_e"; 
			...
			$c ="select * from table_kalibrasi_hua where id_qm='$id_qm'";
		$q = mssql_query($c);
		$n = mssql_num_rows($q);
		
		if ($n <= 0) {
			$add="insert into table_kalibrasi_hua(notes1,notes2,notes3,notes4,notes5,notes6,notes7,q1,q2,q3,q4,q5,q6,q7,save_q1,save_q2,save_q3,save_q4,save_q5,save_q6,save_q7,status_save,id_pribadi_user,id_pribadi_observer,date_saved,tot_score,id_unit,lb,id_qm,notes_score,id_kalibrasi)
		  values('$s_notes1','$s_notes2','$s_notes3','$s_notes4','$s_notes5','$s_notes6','$s_notes7','$sc1','$sc2','$sc3','$sc4','$sc5','$sc6','$sc7','$tot1','$tot2','$tot3','$tot4','$tot5','$tot6','$tot7','complete','$id_pribadi_user','$observer',getdate(),'$save_total','$kd_unit','$id_lb','$id_qm','$notes_score','$SES_hendi')";
			mssql_query($add);
		//echo "$add";
		 
		 
		 $date_saved=date("m/d/Y H:i:s");		  
	 $add2="update table_qm_hua set id_kalibrasi='$SES_hendi', kalibrasi='$agree' where id_qm='$id'";
		mssql_query($add2);
		  //echo "$add2";
			}	
