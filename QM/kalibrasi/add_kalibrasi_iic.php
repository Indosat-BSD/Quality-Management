...
<?
		  ...
		  if ($Submit)
	  {
		...
		$c ="select * from table_kalibrasi_iic where id_qm='$id'";
		$q = mssql_query($c);
		$n = mssql_num_rows($q);
		
		if ($n <= 0) {
			$add="insert into table_kalibrasi_iic(notes1,notes2,notes3,notes4,notes5,notes6,notes7,notes8,notes9,notes10,notes11,q1,q2,q3,q4,q5,q6,q7,q8,q9,q10,q11,save_q1,save_q2,save_q3,save_q4,save_q5,save_q6,save_q7,save_q8,save_q9,save_q10,save_q11,status_save,id_pribadi_user,id_pribadi_observer,date_saved,tot_score,id_unit,lb,id_qm,notes_score,id_kalibrasi)
		  values('$s_notes1','$s_notes2','$s_notes3','$s_notes4','$s_notes5','$s_notes6','$s_notes7','$s_notes8','$s_notes9','$s_notes10','$s_notes11','$sc1','$sc2','$sc3','$sc4','$sc5','$sc6','$sc7','$sc8','$sc9','$sc10','$sc11','$tot1','$tot2','$tot3','$tot4','$tot5','$tot6','$tot7','$tot8','$tot9','$tot10','$tot11','complete','$id_pribadi_user','$observer',getdate(),'$save_total','$kd_unit','$id_lb','$id','$notes_score','$SES_hendi')";
			mssql_query($add);
		//echo "$add";
		 
		 
		 $date_saved=date("m/d/Y H:i:s");		  
		 $add2="update table_qm_iic set id_kalibrasi='$SES_hendi', kalibrasi='$agree' where id_qm='$id'";
		  mssql_query($add2);
		  //echo "$q_addTemp";
		}
