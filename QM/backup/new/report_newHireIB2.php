...
<form action="<? $PHP_SELF ?>" method="post" name="satu" enctype="multipart/form-data" onKeyPress="return noenter()">
  <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
    <tr class="content">
		<td width="20%">
	   					<?php
							 echo "<a class='btn btn-leave' href='?index=report_newHireIB'>Karyawan</a>";
							 echo "<a class='btn btn-pilih' href='?index=report_newHireIB2'>Seleksi</a>";
						?></td>
		<td width="80%">&nbsp;</td>
	</tr>
	<tr class="content">
    ...
    <tr class="content">
      ...
      <td><select name="report_by" id="report_by" class="box" onchange="satu.submit()">
        ...
        <?php
			$katahh=array('Agent','Observer','Skala');
    ...
    <tr class="content">
      ...
      <td><select name="c_observer" class="box" id="c_observer" >
        <option value="" selected="selected">Select All
        <?php

//mengambil nama si pemberi nilai		
include "konek_qm.php";
...
select id_qm from (
	...
	$itung="[sp_count4]'table_qm_newHireIB','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala'";
...
//echo "$q1_yes_jum";
	...
	if((isset($searchButton) and $jum_tot<>"0") or ($kd_del=="ok"and $jum_tot<>"0"))
					...
					//var_dump($skala);
					if($c_observer==""){$obs="%%";}else{$obs=$c_observer;}
					$q_user="[sp_search4]'table_qm_newHireIB','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala' ";
					//echo $q_user;
					//echo "456456";
...
function confirmDelete2()
{
    ...
    <?
        ...
        echo"       <td>$kalibrasi</td>  
				...
				//$queryhendi="[sp_reportqm_newhire]'table_qm_newHireIB','$tgl1','$tgl2','$report_by','$obs','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala','0'";
    ...
    <tr>
      ...
      <td>
		...
		$queryhendi="[sp_reportqm_newhire]'table_qm_newHireIB','$tgl1','$tgl2','$report_by','$obs','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala','0'";
		  ...
		  //$queryhendi="[sp_reportqm_inbound2]'table_qm_newHireIB','$tgl1','$tgl2','$report_by','$c_observer','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala',''";
	  		...
	  		if ($select_region !="") {
	  			if ($report_by=="Unit")
					  ...
					  $stot_rep="select count(*) as jum_s_rep from db_qm.dbo.table_qm_newHireIB_detail c
					   ...
					   and a.id_unit like '%$kd_unit%' and  convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2' and a.flag_status=0";
	   				}
	  			if ($report_by=="Departement")
					 ...
					 and (a.id_unit like '%$cc_mentari%' or a.id_unit like '%$cc_im3%' or a.id_unit like '%$cc_pp%') 
					  ...
					  where b.region ='$select_region' and  a.status_save='complete' and repeat='yes'
					  and (a.id_unit like '%$cc_mentari%' or a.id_unit like '%$cc_im3%' or a.id_unit like '%$cc_pp%')
					  and  convert(varchar,a.date_saved,112) between '$tgl1' and'$tgl2' and a.flag_status=0"; 
					}
			}
			else {
	  			if ($report_by=="Unit")
...
select id_qm from (
					...
					from db_qm.dbo.table_qm_newHireIB a with(nolock) 
					inner join db_qm.dbo.table_qm_newHireIB_detail b with(nolock) on a.id_qm=b.id_qm 
					where a.status_save='complete' and b.fcr='fcr' and convert(varchar,a.date_saved,112) between '$tgl1' and '$tgl2' and a.flag_status=0
				    group by a.id_qm 
...
inner join db_qm.dbo.table_qm_newHireIB_detail b with(nolock) on a.id_qm=b.id_qm
 where a.status_save='complete' and convert(varchar,a.date_saved,112) 
  between '$tgl1' and '$tgl2' and a.flag_status=0
...
";   
    ...
    <tr>
      ...
      <td width="68">
		<?  
					if($c_observer==""){$obs="%%";}else{$obs=$c_observer;}
					$stot_hh="[sp_handling_newhire]'table_qm_newHireIB','$report_by','$tgl1','$tgl2','$kd_unit','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region','$skala','$departemen_skala','$unit_skala','0'";
			//echo"$stot_hh";
