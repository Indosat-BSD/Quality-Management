...
<!-- Report By Unit -->	  	  
    ...
    <tr class="content">
      <td>Departemen</td>
      <td><select name="kd_departemen" id="select" onchange="satu.submit()" class="box" >
...
<!-- Report By Departement -->
    ...
    <tr class="content">
      <td>Departemen</td>
      <td>
		<input name="cc_mentari" type="checkbox" id="cc_mentari" value="CCC01" <? if ($cc_mentari=="CCC01") {echo "checked";} else {$cc_mentari==''; echo "";} ?>/>
      Postpaid Collection 
...
inner join hrms.dbo.tb_auth c on a.id_pribadi_observer=cast(c.id_data_pribadi as varchar) --where c.qm='admin' 
order by z.nama";
//echo"$q_quizCombo";
$r_observerCombo = mssql_query( $q_observerCombo );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='report/export_credit.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region'>export to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
  ...
  <thead>
    ...
    <?
		//echo "$c_observer";
		//echo "$select_region";
		//echo"mentari : $cc_mentari<br>";
		//echo"im3: $cc_im3<br>";
		//echo"Divisi : $katadivisi[$i] <br>";
		//echo" Departement:$katadepartemen[$i] <br>";
		//echo" Kode Departement : $kd_departemen <br>";
		//echo"Unit : $kata[$i]<br>";
		//echo"Kode Unit : $kd_unit <br>";
	include "konek_qm.php";
	//prosedur
	if($c_observer==""){$obs=0;}else{$obs=$c_observer;}
	if ($date_eva1 != "" && $date_eva2 != "") {
	$itung="[sp_countAdd_kalibrasi]'table_qm_credit','$tgl1','$tgl2','$report_by','%$kd_unit%','%$cc_mentari%','%$cc_im3%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region'";
	 $queryitung=mssql_query($itung);
	 $hasilitung=mssql_fetch_array($queryitung);
	 $jum_tot=$hasilitung["jum_s"];
	//echo $itung;
	//echo " <br>$kd_division : Divisi :$katadivisi[$i] ";
	//echo" <br>$kd_departemen :Departement : $katadepartemen[$i]";
	//echo " <br>$kd_unit : Unit : $kata[$i] ";
	//end edit
//echo "$q1_yes_jum";
	...
	if((isset($searchButton) and $jum_tot<>"0") or ($kd_del=="ok"and $jum_tot<>"0"))
					...
					$q_user="[sp_searchAdd_credit]'table_qm_credit','$report_by','%$kd_unit%','$tgl1','$tgl2','%$cc_mentari%','%$cc_im3%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
					//echo $q_user;
					//echo "456456";
