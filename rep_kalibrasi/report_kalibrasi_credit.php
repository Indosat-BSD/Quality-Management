...
from db_qm.dbo.table_kalibrasi_credit_control a with(nolock) inner join
...
$r_observerCombo = mssql_query( $q_observerCombo );
    ...
    <tr class="content">
      ...
      <td><input name="searchButton" type="submit" id="searchButton" value="Search"  class="btn btn-primary"/>
		  ...
		  echo "<a class='btn btn-warning' href='rep_kalibrasi/export_kalibrasi_credit.php?cc_mentari=$cc_mentari&cc_im3=$cc_im3&cc_pp=$cc_pp&cc_matrix=$cc_matrix&cc_m2=$cc_m2&cc_ftm=$cc_ftm&report_by=$report_by&nik_agent=$nik_agent&c_observer=$c_observer&kd_unit=$kd_unit&date_eva1=$tgl1&date_eva2=$tgl2&select_region=$select_region'>export validasi credit to excel</a>";
		  ?></td>
    </tr>
  </table>
  <p>&nbsp;</p>
	...
	if ($date_eva1 != "" && $date_eva2 != "") {
	 ...
	 $itung="[sp_count_kalibrasi_credit]'table_kalibrasi_credit_control','table_qm_credit','$tgl1','$tgl2','$report_by','%$kd_unit%','$cc_mentari','$cc_im3','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$obs','%$nik_agent%','$select_region'";
...
//echo "$q1_yes_jum";
  ...
  <thead>
	...
	<?
					$q_user="[sp_search_kalibrasi_credit]'table_kalibrasi_credit_control','table_qm_credit','$report_by','%$kd_unit%','$tgl1','$tgl2','$cc_mentari','$cc_im3','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
					// echo"$q_user";
...
// $total_score=$save_q1+$save_q2+$save_q3+$save_q4+$save_q5+$save_q6+$save_q7+$save_q8+$save_q9+$save_q10+$save_q11;
					...
					function confirmDelete2()
					{
					  ...
					  <a href="<? echo "del_kalibrasi.php?report_by=$report_by&kode_del=detail_credit&id_qm=$id_qm&kd_division=$kd_division&kd_departemen=$kd_departemen&kd_unit=$kd_unit&date_eva1=$date_eva1&date_eva2=$date_eva2&c_observer=$observer&id=$id"; ?>" onclick="return confirmDelete2();">
					  <?
					  	 echo "delete</a></td>";
    ...
    <tr>
      ...
      <td>
		<?
		  ...
		  $queryhendi="[sp_reportqm_verifikasiHQ]'table_kalibrasi_credit_control','$tgl1','$tgl2','$report_by','%$c_observer%','%$nik_agent%','%$cc_mentari%','%$cc_im3%','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','%$kd_unit%','%%','$select_region','$skala','$departemen_skala','$unit_skala',''";
