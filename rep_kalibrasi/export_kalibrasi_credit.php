...
<table width="536%" border="1" align="center" cellspacing="1" class="table table-bordered table-infinite">
    <thead>
	 ...
	 <? if ($SES_KODE_LEVEL=="admin" or $SES_KODE_LEVEL=="mitra" or $SES_KODE_LEVEL=="validator"){?>
		...
		<th width="2%" rowspan="3" class="head0"><div align="center">validation</div></th>
		<th width="2%" rowspan="3" class="head0"><div align="center">Validator</div></th>
	</tr>
  ...
  <?
	include "../konek_qm.php";
	 ...
	 $itung="[sp_count_kalibrasi_credit]'table_kalibrasi_credit_control','table_qm_credit','$date_eva1','$date_eva2','$report_by','%$kd_unit%','$cc_mentari','$cc_im3','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
	 // echo $itung;
	 $queryitung=mssql_query($itung);
	 $hasilitung=mssql_fetch_array($queryitung);
	 $jum_tot=$hasilitung["jum_s"];

					$q_user="[sp_search_kalibrasi_credit]'table_kalibrasi_credit_control','table_qm_credit','$report_by','%$kd_unit%','$date_eva1','$date_eva2','$cc_mentari','$cc_im3','%$cc_pp%','%-%','%-%','%-%','%-%','%-%','%-%','%-%','$c_observer','%$nik_agent%','$select_region'";
					//echo"$q_user";
